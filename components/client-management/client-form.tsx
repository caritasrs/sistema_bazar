...
export function ClientForm({ onSuccess, onCancel }: ClientFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (cpfExists?.exists && cpfExists.user !== "CPF inválido") {
      setMessage({
        type: "error",
        text: `Este CPF já está cadastrado para ${cpfExists.user}`,
      })
      return
    }

    if (cpfExists?.user === "CPF inválido") {
      setMessage({ type: "error", text: "CPF inválido. Verifique o número digitado." })
      return
    }
    ...
    if (emailExists?.exists) {
      setMessage({
        type: "error",
        text: `Este e-mail já está cadastrado para ${emailExists.user}`,
      })
      return
    }
    ...
    if (phoneExists?.exists) {
      setMessage({
        type: "error",
        text: `Este telefone já está cadastrado para ${phoneExists.user}`,
      })
      return
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {emailExists?.exists && (
            <p className="text-xs text-red-400 mt-1 font-semibold">E-mail já cadastrado para: {emailExists.user}</p>
          )}
        ...
        <div>
          ...
          {phoneExists?.exists && (
            <p className="text-xs text-red-400 mt-1 font-semibold">Telefone já cadastrado para: {phoneExists.user}</p>
          )}
